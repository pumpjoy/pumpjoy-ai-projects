...
def fetch_stock_data(symbol, function_name='TIME_SERIES_DAILY'):
    ...
    # --- Crucial API Skill: Error Handling ---
...
#     # The key for the time series data might change based on the function
#     # For TIME_SERIES_DAILY, it's typically 'Time Series (Daily)'
#     time_series_key = next(iter(raw_data.keys())) 

#     if time_series_key not in raw_data:
#         print("Invalid or no stock data returned.")
#         return pd.DataFrame() # Return an empty DataFrame

#     data = raw_data[time_series_key]

#     # Create DataFrame from nested dictionary, orienting by the index (dates)
#     df = pd.DataFrame.from_dict(data, orient='index')

#     # Clean up column names and convert to proper data types
#     df.columns = ['Open', 'High', 'Low', 'Close', 'Volume']
#     df.index.name = 'Date'
#     df.index = pd.to_datetime(df.index)

#     # Convert financial data columns to numeric
...
def process_data_to_dataframe(raw_data):
    # --- 1. Isolate the Time Series Data ---
    ...
    # --- 2. Create DataFrame from the nested dictionary ---
    ...
    # --- 3. Clean up column names and index ---
...
def visualize_data(df, symbol):
    # Plotly is great because it creates an interactive HTML file
